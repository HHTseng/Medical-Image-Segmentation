    ...
    for idx, (images, dsc) in enumerate(zip(zip(normalized_inputs, labels, predictions), DSCs)):
        ...
        for i in range(N):
            ...
            if DSCs is not None:
                ...
                dsc_caption = 'DSC: {:.3f}'.format(dsc[i, -1])
...
def train(train_loader, net, criterion, optimizer, epoch, config, restore, visualize, device):
    ...
    for batch_idx, (X, y) in enumerate(train_loader):
        ...
        assert X.size()[2:] == y.size()[1:]   # check input image size  = label image size
        ...
        if random.random() > config.train_img_sample_rate:
            ...
            X_batch_disp.append(X.data.squeeze_(0).cpu())
            y_batch_disp.append(y.data.squeeze_(0).cpu().numpy())
        ...
        # over all evaluation
        DSC_batch_disp.append(DSC(y_pred, y.data.squeeze_(0).cpu().numpy(), config.num_classes))
...
ckpt_path = '/nfs/turbo/umms-ielnaqa/HTseng/Model_results/Segmentation_results/model_ckpt' 
data_path = '/nfs/turbo/umms-ielnaqa/HTseng/Data/LITS_challenge/Train/'
exp_name = 'LITS_UNET'
...
class Config(object):
    ...
    train_img_sample_rate = 1        # randomly sample some training results to display
    validation_img_sample_rate = 1   # randomly sample some validation results to display
...
train_params = {'batch_size': config.batch_size, 'shuffle': True, 'num_workers': 4, 'pin_memory': True} if use_cuda else {}
valid_params = {'batch_size': config.batch_size, 'shuffle': False, 'num_workers': 4, 'pin_memory': True} if use_cuda else {}
...
for epoch in range(config.epoch_num):
    # training model
    train(train_loader, net, loss_func, optimizer, epoch, config, no_transform, visualize, device)
